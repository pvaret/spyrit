...
SETTINGS_SCHEMA = {
    "keys": (
        ( "app.name",
          { "serializer": Str(), "default": "Spyrit" } ),
        ( "app.version",
          { "serializer": Str(), "default": "0.5dev" } ),
        ( "log.file",
          { "serializer": Str(), "default": "[WORLDNAME]-%Y.%m.%d.log" } ),
        ( "log.dir",
          { "serializer": Str(), "default": LOG_DIR } ),
        ( "log.autostart",
          { "serializer": Bool(), "default": False } ),
        ( "log.ansi",
          { "serializer": Bool(), "default": False } ),
        ( "ui.style",
          { "serializer": Str(), "default": False } ),
        ( "ui.window.min_size",
          { "serializer": Size(), "default": "640x480" } ),
        ( "ui.window.alert",
          { "serializer": Bool(), "default": True } ),
        ( "ui.toolbar.icon_size",
          { "serializer": Int(), "default": 24 } ),
        ( "ui.view.split_scroll",
          { "serializer": Bool(), "default": True } ),
        ( "ui.view.paging",
          { "serializer": Bool(), "default": True } ),
        ( "ui.view.font.name",
          { "serializer": Str(), "default": default_font } ),
        ( "ui.view.font.size",
          { "serializer": Int(), "default": 0 } ),
        ...
        ( "ui.input.max_history",
          { "serializer": Int(), "default": 0 } ),
        ( "ui.input.save_history",
          { "serializer": Int(), "default": 10 } ),
    ),
    "sections": (
...
DESCRIPTIONS = {
    ...
    "ui.input.save_history":
        "length of input history to keep between sessions",
    ...
    SHORTCUTS + ".disconnect":
        "shortcut: disconnect from the current world",
    ...
    SHORTCUTS + ".home":
        "shortcut: scroll to the beginning of output",
